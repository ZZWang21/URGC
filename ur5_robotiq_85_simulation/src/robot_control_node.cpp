...
static const std::string APP_DIRECTORY_NAME = "/catkin_ws/src/.ur5_robotiq_85_simulation";
...
int main(int argc, char **argv)
{
  ...
  // read JSON files from ~/.ur5_robotiq_85_simulation
  ...
  if (fs::is_directory(home))
  {
    ...
    if (!fs::exists(app_directory) && !fs::is_directory(app_directory))
    {
      ...
      // Create ur5_robotiq_85_simulation directory
      std::string path(getenv("HOME"));
      path += "/catkin_ws/src/.ur5_robotiq_85_simulation";
